import { escapeAllCharacters, unslash, replaceAllUnescaped, searchFirstUnescaped, } from "./utils.js";
export class DatePHP extends Date {
    ...
    format(format) {
        ...
        if (searchFirstUnescaped(format, "W") !== -1) {
            console.warn("The W character is currently not supported. It has not been substituted or removed.");
        }
        // Month
        format = replaceAllUnescaped(format, "F", months[month]);
        format = replaceAllUnescaped(format, "m", (month + 1).toString().padStart(2, "0"));
        format = replaceAllUnescaped(format, "M", months[month].slice(0, 6));
        format = replaceAllUnescaped(format, "n", (month + 1).toString());
        format = replaceAllUnescaped(format, "t", new Date(year, month + 1, 0).getDate().toString());
        // Year
        ...
        if (searchFirstUnescaped(format, "o") !== -1) {
            console.warn("The o character is currently not supported. It has not been substituted or removed.");
        }
        ...
        format = replaceAllUnescaped(format, "A", escapeAllCharacters(meridiem.toUpperCase()));
        format = replaceAllUnescaped(format, "g", smallHours.toString());
        format = replaceAllUnescaped(format, "G", hours.toString());
        format = replaceAllUnescaped(format, "h", smallHours.toString().padStart(2, "0"));
        format = replaceAllUnescaped(format, "H", hours.toString().padStart(2, "0"));
        format = replaceAllUnescaped(format, "i", minutes.toString().padStart(2, "0"));
        format = replaceAllUnescaped(format, "s", seconds.toString().padStart(2, "0"));
        format = replaceAllUnescaped(format, "v", milliseconds.toString().padStart(3, "0"));
        if (searchFirstUnescaped(format, "B") !== -1) {
            console.warn("The B character is currently not supported. It has not been substituted or removed.");
        }
        if (searchFirstUnescaped(format, "u") !== -1) {
            console.warn("The u character is not supported because of JS Date limitations. It has not been substituted or removed.");
        }
        // Timezone
        if (timezoneMinutesOffset === 0) {
            ...
            format = replaceAllUnescaped(format, "O", `${timezoneMinutesOffset < 0 ? "+" : "-"}${Math.abs(exactHourLength)
                .toString()
                .padStart(2, "0")}${remainderMinutes.toString().padStart(2, "0")}`);
            format = replaceAllUnescaped(format, "P", `${timezoneMinutesOffset < 0 ? "+" : "-"}${Math.abs(exactHourLength)
                .toString()
                .padStart(2, "0")}:${remainderMinutes.toString().padStart(2, "0")}`);
            format = replaceAllUnescaped(format, "p", `${timezoneMinutesOffset < 0 ? "+" : "-"}${Math.abs(exactHourLength)
                .toString()
        ...
        if (searchFirstUnescaped(format, "e") !== -1) {
            console.warn("The e character is currently not supported. It has not been substituted or removed.");
        }
        if (searchFirstUnescaped(format, "I") !== -1) {
            console.warn("The I character is currently not supported. It has not been substituted or removed.");
        }
        if (searchFirstUnescaped(format, "T") !== -1) {
            console.warn("The T character is currently not supported. It has not been substituted or removed.");
        }
        if (searchFirstUnescaped(format, "Z") !== -1) {
            console.warn("The Z character is currently not supported. It has not been substituted or removed.");
        }
        format = replaceAllUnescaped(format, "U", (this.valueOf() / 1000).toString());
        return unslash(format);
    }
    static parseString(dateString, formatString) {
        if (formatString !== "Y-m-d\\TH:i:s.vp" && formatString !== "ISO-8601") {
            console.error("parseDateUsingPHPDateFormat: Currently only ISO-8601 is supported.");
            return new DatePHP();
        }
        ...
        return new DatePHP(Date.UTC(year, month, day, hours, minutes, seconds, milliseconds));
    }
}

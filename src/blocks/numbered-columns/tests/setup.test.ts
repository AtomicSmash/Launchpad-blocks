...
import { numberedColumnsFixtures } from "./fixture";
...
setup(`${blockName} test setup`, async ({ page }) => {
	...
	await adminHelper.addContentByFixture(`${slugify(blockName)}-test-page`, [
		numberedColumnsFixtures.many,
		numberedColumnsFixtures["4"],
		numberedColumnsFixtures["3"],
		numberedColumnsFixtures["2"],
		numberedColumnsFixtures["1"],
	]);
});
